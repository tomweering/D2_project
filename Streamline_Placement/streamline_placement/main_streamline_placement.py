...
# n_seed_points = 4 Radius = 0.5 dsep = 0.25 mesh = mesh10_3np pyvista find vector at point#define separation
# distance, dsep, between streamlines and the checking parameter to avoid colisions, dtest (a proportional param.
# between 0 and 1) dsep = 0.5 #cell unit lengths dtest = 1 #proportion parameter u_list = u_list v_list = v_list
# w_list = w_list
"""--------------------------------------------------------------"""
"""
...
def streamline_placement(init_point, mesh_pyvista, mesh_scipy, u_list, v_list, w_list, integration_direction,
    ...
    # streamline1 = mesh_pyvista.streamlines(vectors="vectors", source_center=None, source_radius=None, n_points=1,
    # start_position=init_point.points[0], return_source=False, pointa=None,pointb=None, progress_bar=False) convert
    # streamline into a set of points
    ...
    while len(queue_streamlines) > 0:
        ...
        while len(queue_base_points) > 0:
            ...
            # print("number of filtered base points", len(filtered_seed_points)) print("Filtered Seed points",
            # filtered_seed_points) integrate from the filtered seed_points while after each one, adding the
            # resulting streamline (as points) to occupied_points, print_lines and queue if len(filtered_seed_points)
            # == 0: print("empty")
            ...
            for i in filtered_seed_points:
                ...
                if ((len(streamline_as_points) - 1) * initial_step_length) > (0.5 * min(mesh_pyvista.dimensions)):
                    ...
                    if ((len(streamline_as_points) - 1) * initial_step_length) > (0.5 * min(mesh_pyvista.dimensions)):
                        ...
                        # mesh pyvista dimensions divided by two", (0.5 * min(mesh_pyvista.dimensions)))
                        occupied_points = np.append(occupied_points, streamline_as_points, axis=0)
                        print_lines.append(streamline_as_points)
                        queue_streamlines.append(streamline_as_points)
...
# LOOP: while there are streamlines in queue: ----assign streamline from queue to a variable representing the current
# streamline ----append streamline (as point data) to a list of streamlines for storage (this will be the final
# print-line list) ----add point data of streamline into unstructured grid representing 'taken' points ----remove
# streamline from queue ----initiate point queue (empty) ----make queue of all possible seed points and filter them
# based on other streamlines ----LOOP: ----While point in seed points queue (starting from a logical first one,
# probably near the start of the base streamline): ----#----assign point as variable seed_point ----#----assign this
# point to set of seed-points ----#----integrate the streamline from these seed-points, taking care to avoid
# collisions (using the same local collision function) ----#----remove current point from queue ----#----append new
# streamline to queue
