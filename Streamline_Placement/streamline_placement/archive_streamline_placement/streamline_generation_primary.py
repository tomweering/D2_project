...
stream,src = mesh10_3.streamlines(
    'vectors', return_source=True, terminal_speed=0.0, n_points=1, start_position= (0,0,0))
...
p.add_points(points, render_points_as_spheres=True, point_size=5)#plotting the points you get from the streamlines to check them
...
seed_points = pv.PointSet(seed_points)
"""
streamline = mesh10_3.streamlines_from_source(seed_points, vectors="vectors", integration_direction="forward", initial_step_length=1.0, step_unit='cl', min_step_length=1.0, max_steps=2000, terminal_speed=0.0)
...
p.add_mesh(mesh10_3.outline())
#p.add_points(points, render_points_as_spheres=True, point_size=5)#plotting the points you get from the streamlines to check them
p.add_mesh(streamline.tube(radius=0.01))
p.show()
"""
